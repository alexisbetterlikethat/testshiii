...
function RageSystem.isTargetOverVoid(targetRoot, targetCharacter)
    ...
    local result = workspace:Raycast(targetRoot.Position, Vector3.new(0, -math.max(RageSettings.VoidRaycastLength or 1000, 1000), 0), params)
...
function RageSystem.expandHitbox(character)
    ...
    if head and head:IsA("BasePart") then
        if head.Size.X < 10 then -- Only expand if not already expanded
